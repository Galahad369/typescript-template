...
export async function main(ns) {
    ...
    function select(node) {
      while (node.children.length > 0) {
        ...
        for (const c of node.children) {
          ...
          // Keep the tree policy aligned with the side to move at this node.
          const exploitation =
            node.player === "X"
  ...
  function scoreMoveHeuristic(
    ...
    // STRONG penalty for filling own eyes (hard ban, never waived in endgame)
    if (move.captured === 0 && isTrueEye(board, move.row, move.col, player)) {
      score -= 50000000; // Hard-ban self-eye fills unless they capture
    }
    ...
    // In endgame (moves 15+), allow isolated captures of enemy territory for area gain
    ...
    try {
      ...
      // Moves 15-20: aggressive capture (500k) - we are in endgame, maximize area
  ...
  function scoreMoveRough(
    ...
    // Extended to move 14 to prevent aggressive invasion in mid-game (cause of 0-stone losses)
    // In endgame (moves 15+), remove bonus to allow isolated captures of enemy territory
    if (moveCount >= 3 && moveCount <= 14) {
      let friendlyNeighbors = 0;
      for (const [nextRow, nextCol] of neighbors(move.row, move.col)) {
        if (
    ...
    if (territoryOwner === "X" && move.captured === 0) {
      ...
      // In late endgame (moves 15+), aggressively capture enemy territory
      if (moveCount >= 21) score += 800000;
  ...
  function chooseMove(board, moveCount) {
    ...
    const finalScoreOrig = estimateFinalScore(board, origControlled, komi);

    function allowSacrificeMove(move) {
      ...
      try {
        ...
        return finalScoreSim >= finalScoreOrig - (moveCount >= 12 ? 1.5 : 0.5);
      } catch {
        return false;
      }
    }
    ...
    */
    ...
    let candidateMoves = analyzedMoves.filter((m) => {
      try {
        ...
        if (isSelfEyeFillMove(m) && !allowSacrificeMove(m)) return false;
        if (isSelfFoolingMove(m) && !allowSacrificeMove(m)) return false;
        ...
        if (
          ...
          if (allowSacrificeMove(m)) return true;
          return false;
        }
  ...
  // ===== EYE STRATEGY FUNCTIONS =====
  // In Go, having 2 eyes guarantees a group lives. One eye = killable, zero eyes = dead.
  // Strategy: (1) Create own eyes to secure life, (2) Block enemy eyes to threaten enemy groups

  function isPotentialEye(board, row, col, player) {
    // A potential eye is an empty point that could become an eye if surrounded by the player
    if (board[row][col] !== ".") return false;
    for (const [nextRow, nextCol] of neighbors(row, col)) {
      if (!inBounds(board, nextRow, nextCol)) continue;
      const cell = board[nextRow][nextCol];
      if (cell !== player && cell !== "#") return false; // Not a potential eye if any neighbor is enemy or neutral
    }
    return true;
  }

  function countPotentialEyes(board, player) {
    // Count all empty points that COULD become true eyes if occupied by player
    let count = 0;
    for (let r = 0; r < board.length; r++) {
      for (let c = 0; c < board[0].length; c++) {
        if (isPotentialEye(board, r, c, player)) count++;
      }
    }
    return count;
  }

  function countEyesForGroup(board, row, col, player) {
    // Count actual true eyes (safe points) for the group containing (row, col)
    ...
    while (stack.length > 0) {
      ...
      // Check all neighbors for eyes
      for (const [nextRow, nextCol] of neighbors(r, c)) {
        if (!inBounds(board, nextRow, nextCol)) continue;
  ...
  function wouldCreateEye(board, row, col, player) {
    // Check if playing at (row, col) would create a true eye for the player
    if (board[row][col] !== ".") return false;
    if (!isPotentialEye(board, row, col, player)) return false;
    // After placing, this point becomes a true eye
    return true;
  }

  function wouldBlockEnemyEye(board, row, col, player) {
    // Check if playing at (row, col) blocks enemy from creating an eye there
    const enemy = player === "X" ? "O" : "X";
    if (board[row][col] !== ".") return false;
    // If this point is a potential eye for enemy, filling it blocks them
  ...
  function findMostVulnerableEnemyGroup(board, player, legalMoves) {
    ...
    for (let r = 0; r < board.length; r++) {
      for (let c = 0; c < board[0].length; c++) {
        if (board[r][c] === enemy) {
          const eyeCount = countEyesForGroup(board, r, c, enemy);
          if (eyeCount <= 1) {
